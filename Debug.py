...
while cap and cap.isOpened():
    ...
    for bbox in bbox_id:
        ...
        # Ensure each object has a speed value
        if obj_id not in speeds:
            speeds[obj_id] = 0

        # Check left side of the lane divider (vehicles moving up)
        if cx < lane_divider:  # Vehicles moving UP
            if blue_line_y - offset < cy < blue_line_y + offset:
                if obj_id not in up:
                    up[obj_id] = time.time()
                    print(f"Up Line Crossed: ID {obj_id}, Time: {up[obj_id]}")
            if obj_id in up:
                if red_line_y - offset < cy < red_line_y + offset:
                    ...
                    if obj_id not in counter_up:
                        ...
                        if distance is not None:
                            ...
                            print(f"Speed Calculated for ID {obj_id}: {speeds[obj_id]} Km/h")
                        else:
                            print(f"Distance not defined for camera {camera_option}")
        else:  # Vehicles moving DOWN
            if red_line_y - offset < cy < red_line_y + offset:
                if obj_id not in down:
                    down[obj_id] = time.time()
                    print(f"Down Line Crossed: ID {obj_id}, Time: {down[obj_id]}")
            if obj_id in down:
                if blue_line_y - offset < cy < blue_line_y + offset:
        ...
        # Debugging Text for Each Object
        debug_text = f"ID: {obj_id}, Speed: {int(speeds[obj_id])} Km/h"
        cv2.putText(frame, debug_text, (x, y - 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
